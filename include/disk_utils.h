...
DISKANN_DLLEXPORT int build_disk_index(
    ...
    const uint32_t Lf = 0,
    const bool keep_vamana_graph = false); // default is empty string for no universal label
...
template <typename T>
DISKANN_DLLEXPORT void create_disk_layout_block_shuffling(const std::string base_file,
